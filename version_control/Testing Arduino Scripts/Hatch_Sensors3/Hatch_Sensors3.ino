...
float roll; // IMU roll, pitch, yaw variables in degrees
float pitch;
float yaw;
...
// Initalises USB, transceiver radio(serial2) & GPS(serial1), sets pinmodes, initializes sensors, and starts the BME conversion process.
void setup() { 
  ...
  Serial2.begin(115200, SERIAL_8N1, 16, 17); // Setup UART serial communication port for radio transceiver on pins 16 (RX) and 17 (TX)
  ...
  battery_pi_read.clear();   // Clear battery averaging buffers
  battery_analog_read.clear();

  
///////////////////////////////////////////BME680 ENVIRONMENTAL TEMPERATURE 
   uint8_t rslt = 1;
   while(!Serial); 
   delay(400);
...
void loop() {
   battery_pi_read.addValue(analogRead(pi_bat)); // Add Pi n backup battery sample to avg
   battery_analog_read.addValue(analogRead(backup_bat));

   //// Read IMU data and calculate roll, pitch, yaw angles from quaternion
    // Calculate roll pitch yaw
    
    if (bno08x.getSensorEvent(&sensorValue)) { // Read latest IMU sensor event, returns true if new data is available
    if (sensorValue.sensorId == SH2_ROTATION_VECTOR) { // Check if the event is a rotation vector (quaternion) reading
      // Extract quaternion components from the sensor event
      qw = sensorValue.un.rotationVector.real; // IMU quaternion values representing orientation - not used directly but converted to Euler angles, not currently transmitted to Pi
  ...
  //////////////////////////////////////////////Sends data to Raspbery Pi and prints to screen///////////////////////////////////////////////////////////////////////////
  time_output = millis()-prev_output;
   if (time_output >= print_frequency){
   prev_output = millis();
   Serial.print("h"); // Start of data packet indicator, 'h' for hatch
   Serial.print(",");

   Serial.print(roll, 2); // Print roll, pitch, yaw angles with 2 decimal places to serial, separated by commas
   Serial.print(",");
   ...
   //print environment
   Serial.print(Temperature, 2); // Print temperature, humidity, and pressure with 2 decimal places
   Serial.print(",");
   Serial.print(Humidity, 2);
   Serial.print(",");
   Serial.print(Pressure, 2);
   Serial.print(",");
   Serial.print(voltage_pi);// pi battery reading in voltages 
   Serial.print(",");
   Serial.print(voltage_analog);// reading in voltages 
   Serial.print(",");

   //GPS write
   if(GPS.fix){ // If GPS has a fix, print latitude, longitude, speed (converted from knots to km/h), altitude, and number of satellites. If no fix, print zeros.
   ...
   else{Serial.print("0n,0e,0,0,0");} // If no GPS fix, print zeros for all GPS fields (latitude, longitude, speed, altitude, satellites)
   
   Serial.print("0\n"); // End of data packet indicator, '0' for end, followed by newline character to signal end of line for the Pi's serial parsing. The Pi will read until it sees a newline character to get the full data packet.
   Serial.flush(); // Ensure all serial data is sent before continuing


   /// Receieve from pi and send to transceiver 
   // If there's data from the transceiver, read it and if it's a "stop" command, print "stop" to the Pi. If there's data from the Pi and not from the transceiver, read it
   if(Serial2.available()>0){ 
    ...
    if(Serial.available()>0 && !Serial2.available()>0){ // Only read from Pi if there's no data from transceiver to avoid conflicts
