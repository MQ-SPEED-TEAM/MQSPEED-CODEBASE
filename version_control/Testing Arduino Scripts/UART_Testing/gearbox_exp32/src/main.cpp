...
#define DRIVER_ADDRESS 0  // Driver address according to MS1 and MS2
...
unsigned long print_wait_time = 0;
...
void stepMotor(uint32_t steps, uint32_t stepDelay)
{
  ...
  for (uint32_t i = 0; i < steps; i++)
  {
    ...
    if (stall_on)
    {
      driver.VACTUAL(0); // safety stop (driver reacts to STEP inputs)
      break;
    }
...
void loop()
{
  ...
  // receivedBytes = uart.receiveData(outputData, sizeof(outputData)/sizeof(outputData[0]));
  ...
  if((millis() - print_wait_time) > 10)  // Print log data every 500 ms
  {
    char msg[128] = "";
