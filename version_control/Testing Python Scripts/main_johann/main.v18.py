...
import csv
from microcontroller_readings_2 import SensorDataProcessor
import os
    ...
    def generate_overlay():
        ...
        overlay_dict = {
            ...
            "line1": [f"c: {sensor_data_processor.c}  l: {sensor_data_processor.l}  r: {sensor_data_processor.r}  "
                      ...
                      f"sa: {sensor_data_processor.sa}  g: {sensor_data_processor.g}  cp: {sensor_data_processor.cp}  bg: {sensor_data_processor.bg}"],
...
#     picam2.pre_callback = apply_overlay
    ...
    while True:
        ...
        # Gear Correction Calculations
        if sensor_data_processor.gear_correction_enable:
            # Keep up to date with calibrated real ratios
            ratios_list = [2.81, 3.21, 3.60, 4.09, 4.50, 5.29]

            def within_error(value, reference, tol=0.3):
                return (reference - tol) <= value <= (reference + tol)

            # Stability filter state
            last_stable_gear = 0
            candidate_gear = 0
            candidate_start_time = 0
            STABILITY_TIME = 0.40   # seconds required to confirm a gear

            def detect_gear(cranks_rpm, shaft_rpm):
                nonlocal last_stable_gear, candidate_gear, candidate_start_time

                # 1. Zero-RPM case → no measurement available
                if cranks_rpm <= 0 or shaft_rpm <= 0:
                    return 0

                # 2. Compute ratio
                gear_ratio = cranks_rpm / shaft_rpm

                # 3. Find matching gear (or 0 if none)
                detected = 0
                for idx, ref in enumerate(ratios_list):
                    if within_error(gear_ratio, ref):
                        detected = idx + 1
                        break

                # 4. If no gear matched, reset candidate and return last stable
                if detected == 0:
                    candidate_gear = 0
                    candidate_start_time = 0
                    return last_stable_gear

                # 5. If new candidate gear detected
                if detected != candidate_gear:
                    candidate_gear = detected
                    candidate_start_time = time.time()
                    return last_stable_gear

                # 6. If candidate has been stable long enough
                if time.time() - candidate_start_time >= STABILITY_TIME:
                    last_stable_gear = candidate_gear

                return last_stable_gear

            sensor_data_processor.ag = detect_gear(sensor_data_processor.cr, sensor_data_processor.s)
        else:
            sensor_data_processor.ag = 0

        # gear_ratio = sensor_data_processor.cr/sensor_data_processor.s

        # ratios_list = [1,2,3,4,5,6] #PLACEHOLDER VALUES

        # def within_error(value, reference):
        #     # Returns true if value is same as another value within acceptable error for gear ratio matching
        #     lower = reference - 0.3
        #     upper = reference + 0.3
        #     return lower <= value <= upper

        # # Choose gear depending on ratio

        # for i in ratios_list:
        #     if within_error(gear_ratio, i):
        #         sensor_data_processor.ag = ratios_list.index(i) + 1
        #     else:
        #         sensor_data_processor.ag = 0
        ...
        if GPIO.input(10) == GPIO.LOW:
            ...
            if (millis >(20*printed_times+time_start)):
                ...
                if sensor_data_processor.gear_correction_enable:
                    sensor_data_processor.transmit_gear()
                
                if transmit_count >= 20:
                    transmit_count = 0
                    sensor_data_processor.transmit_hatch()
