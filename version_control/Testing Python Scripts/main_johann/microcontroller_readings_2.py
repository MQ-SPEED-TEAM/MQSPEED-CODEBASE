...
class SensorDataProcessor:
    ...
    def __init__(self) -> None:
        ...
        self.last_log = 0
        self.active_ports = 0
        self.gear_correction_enable = 1 # sets gear correction active (1) or inactive (0)

        # Initialize expected data as a list for comparison in update_attributes function
        self.expected_data = ["c","l","r","cr",
                              "s","ts","sa","g","cp","bg","rl","ph","yw","t","h",
                              "p","bp","ba","la","lo","gs","al","sn","dt","pr", "cd", "ag"]
    ...
    def bytestream_read(self, port=None, ending_line=b"\n"):
        ...
        if port == None or port == 0 or port == "None" or port == b"":
...
#         expected_ports = 3
    ...
    def format_data(self):
        ...
        esp_gear_dictkeys = ["g","cp","bg"]
        esp_hatch_dictkeys = ["rl","ph","yw","t","h", "p","bp","ba","la","lo","gs","al","sn"]
        cal_data_dict = {"ts":self.ts, "dt":self.dt, "pr":self.pr, "cd":self.cd, "ag":self.ag}
...
#         current_data.pop("lo")
        ...
        current_data.pop("gear_correction_enable")
            
        return list(current_data.values())
    
    def transmit_hatch(self):
...
#             print("gearbox gave too much data")
    ...
    def transmit_gear(self):
        """This method transmits the actual gear value to the gear esp using class attribute ag (actual gear)"""

        current_data_string = f"{self.ag}\n"
        current_data_bytes = str.encode(current_data_string)
        
        try:
            # UNCOMMENT OUT ONCE TRANSCIEVER WORKS
            self.espgear.write(current_data_bytes)
            print(f"data sent: {current_data_string}")
        except AttributeError:
            print(f"data packet not sent: {current_data_string}")
        except OSError:
