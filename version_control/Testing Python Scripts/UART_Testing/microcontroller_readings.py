    ...
    def bytestream_read(self, port=None, ending_line=b"\n"):
        ...
        if start_byte_found:
            while current_array < 1:
                    try:
                        byte_read = port.receive_data()  #  Read individual byte from serial port
                    except serial.serialutil.SerialException:
                        return ("port unavailable")
            ...
            port.serial.reset_input_buffer()
...
#         expected_ports = 3
    ...
    def raw_read(self):
        ...
        self.espgear = SafeUART(serial.Serial(port='/dev/ttyAMA3', baudrate=115200, 
                                           timeout=1, parity=serial.PARITY_NONE,
                                           stopbits=serial.STOPBITS_ONE,
                                           bytesize=serial.EIGHTBITS))
        array_tmp = []
        while True:
            byte_read = self.espgear.receive_data()  # Read bytes from serial port
            if byte_read: #if not empty
                byte_char = byte_read.decode()  # Decode bytes to string
                ...
                if byte_read == b"\n":
                    ...
                    if array_tmp[-1] != "\n" or len(array_tmp) < 24 :
                        self.defects += 1
                    self.espgear.serial.reset_input_buffer()
                    array_tmp = []
                    break       
