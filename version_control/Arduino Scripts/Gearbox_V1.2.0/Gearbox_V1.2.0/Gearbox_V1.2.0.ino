...
//float DISTANCE_UP = 95;           //Number of steps per gear change for 1-4
//float DISTANCE_DOWN = 91.5;
//float UPPER_DISTANCE = 95;    //Number of steps per gear change for 4-6
float GEAR_UP_DISTANCE[6] = {
  0.0,
  91.0, // 1 - 2
  92.0, // 2 - 3
  107.0, // 3 - 4
...
float OVERSHOOT=52.0;           //Overshoots the gear to move to the next one     ORIGINAL 37
int HOME_OFFSET = 60;         //Offsets from home to Gear 1
...
////////////////////////////////////////////////////////////////////////////////////////////////*/
      if (digitalRead(SWITCH1)== HIGH && digitalRead(SWITCH2)== HIGH && switch1==0 && switch2==0){  ///////////////////GEAR RESET
        ...
        for(int i = 0; i <  HOME_OFFSET * MS; i++){
            STEP(3000/MS);
        }
...
///////////////////////////////////// CHANGING GEARS /////////////////////////////////////////
      else if(digitalRead(SWITCH1) == HIGH && gear < 6 && switch1 == 0) {
        float moveDistance = GEAR_UP_DISTANCE[gear];

        // Move toward the higher gear 
        digitalWrite(DIR_PIN, HIGH);

        // Accelerate
        for(; START_SPEED_TEMP > 0; START_SPEED_TEMP -= ACCELERATION) {
          STEP((SPEED + START_SPEED_TEMP) / MS);
        }

        // Main Movement plus Overshoot

        for(int i = 0; i < ((moveDistance + OVERSHOOT)* MS - (2 * START_SPEED / ACCELERATION )); i++){
          STEP(SPEED / MS);
        }

        //Decelerate
        for(; START_SPEED_TEMP < START_SPEED; START_SPEED_TEMP += ACCELERATION){
          STEP((SPEED + START_SPEED_TEMP) / MS);
        }

        // Move backwards by the overshoot amount
        if(OVERSHOOT > 0){
          START_SPEED_TEMP = START_SPEED;
          digitalWrite(DIR_PIN, LOW);
          ...
          // Main overshoot return movement
      ...
      else if (digitalRead(SWITCH2) == HIGH && gear > 1 && switch2 == 0){
        // gear - 1 selects the matching transition 
        float moveDistance = GEAR_DOWN_DISTANCE[gear - 1];

        //Move toward the lower gear
        digitalWrite(DIR_PIN, LOW);
        ...
        // Main movement
        for (int i = 0; i < (moveDistance * MS - (2 * START_SPEED / ACCELERATION)); i++){
          STEP(SPEED / MS);
        }
        // Decelerate 
        for(; START_SPEED_TEMP < START_SPEED; START_SPEED_TEMP += ACCELERATION){
