...
float OVERSHOOT=37;           //Overshoots the gear to move to the next one for 1-4
float UPPER_OVERSHOOT= 60;    //Overshoots the gear to move to the next one for 4-6
...
////////////////////////////////////////////////////////////////////////////////////////////////*/
      if (digitalRead(SWITCH1)== HIGH && digitalRead(SWITCH2)== HIGH && switch1==0 && switch2==0){  ///////////////////GEAR RESET
        ...
        for (int i=0; i<((UPPER_DISTANCE+UPPER_OVERSHOOT)*MS-(2*START_SPEED/ACCELERATION)); i++){
          STEP(SPEED/MS); 
        } //Takes steps for gear up
        for(;START_SPEED_TEMP<START_SPEED; START_SPEED_TEMP=START_SPEED_TEMP+ACCELERATION){
          STEP((SPEED+START_SPEED_TEMP)/MS);
        ...
        for (int i=0; i<(UPPER_OVERSHOOT*MS-(2*START_SPEED/ACCELERATION)); i++){
          STEP(SPEED/MS); 
        } //Takes steps for gear up
        for(;START_SPEED_TEMP<START_SPEED; START_SPEED_TEMP=START_SPEED_TEMP+ACCELERATION){
          STEP((SPEED+START_SPEED_TEMP)/MS);
...
if(Serial.available()>0 && !Serial2.available()>0){
      gear_confirm_data = Serial.readStringUntil('\n');
      gear_confirm_data.trim();
      if(gear_confirm_data==2){
        }; //2 is designated index for "shift up"

      if(gear_confirm_data==1){
        }; //1 is designated index for "shift down"
